...
vector<vector<int>> makeTree(int n, vector<vector<int>> &links) {
    vector<vector<int>> tree(n, vector<int>(0)); //트리 할당
    for (int i = 0; i < links.size(); i++)
        tree[links[i][0]].push_back(links[i][1]); //값 넣어주기
    return tree; //트리 리턴
}

int fillDp(vector<int> &sales, vector<vector<int>> &tree, int node, bool flag) {
    if (dp[node][flag] != -1) //이미 탐색했던 상태
        return dp[node][flag]; //dp리턴
    ...
    if (flag) //팀장 참석
        return dp[node][flag] += sales[node - 1]; //매출 더해줌
...
int solution(vector<int> sales, vector<vector<int>> links) {
    int v = sales.size(); //팀원의 수
    dp.assign(v + 1, vector<int>(2, -1)); //-1로 벡터 할당
...
int main() {
    ...
    vector<vector<int>> links = {{10, 8},
                                 ...
                                 {10, 2}};
    //관계도
    int ans = solution(sales, links); //풀이
    cout << ans;
}