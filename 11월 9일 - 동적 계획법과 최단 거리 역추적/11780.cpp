...
vector<int> back(int s, int x, vector<vector<int>> &path) { //x: 도착점부터
    vector<int> result(0); //역추적해서 넣어줄 벡터
    while (x != 0) {
        result.push_back(x); //뒤에서 부터 차례로 추적해서 넣어줌
        x = path[s][x]; //다음 경로
    }
    return result; //리턴 
}
...
vector<vector<int>> allPath(int n, vector<vector<int>> &path) {
    ...
    for (int i = 1; i <= n; i++) { //시작점(i)
        for (int j = 1; j <= n; j++) { //도착점(j)
            if (!path[i][j]) //경로가 없으면
                continue; //넘어감
            vector<int> result = back(i, j, path); //역추적해서 저장
            ans[(i - 1) * n + (j - 1)] = result; //답에 저장
        }
    }
    return ans; //리턴
}
...
int main() {
    ...
    for (int i = 0; i < ans.size(); i++) {
        int size = ans[i].size();
        cout << size << ' '; //최소 비용에 포함되는 도시의 개수
        for (int j = size - 1; j >= 0; j--)
            cout << ans[i][j] << ' '; //도시로 가는 경로
        cout << '\n';
    }
    return 0;
}